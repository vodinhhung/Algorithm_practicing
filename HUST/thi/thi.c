...
Inputdata()
{
    ...
    fflush(stdin);
    scanf("%f %f %f %f  %f %f",&a6,&a5,&a4,&a3,&a2,&a1);
}

float f(float x) /* Ham f(x) */
{
    float p;
    p = a6*x*x*x*x*x + a5*x*x*x*x + a4*x*x*x + a3*x*x + a2*x +a1;
    return((float) p);
}
float f1(float x) /* Dao ham cap 1 cua ham f(x) */
{
    float p;
    p = 5*a6*x*x*x*x + 4*a5*x*x*x + 3*a4*x*x + 2*a3*x +a2;
    return((float) p);
}
float f2(float x) /* Dao ham cap 2 cua ham f(x) */
{
    float p;
    p = 20*a6*x*x*x + 12*a5*x*x + 6*a4*x + 2*a3;
    return((float) p);
}
